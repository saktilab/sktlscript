...
# parser.add_argument('-es14','--es14', type=float,  help='Surface energy for surface 14')
...
for line in f:
    ...
    esurf.append(float(data[1]))
 
for mill in surfaces :
    miller = [int(x) for x in mill]
    indexes.append(miller)
lc = opt.lc
size = opt.natom

atoms = wulff_construction(opt.element, indexes, esurf, size, opt.unitcell, opt.rounding, latticeconstant=lc)
...
wulffshape = WulffShape(lattice, indexes, esurf)
