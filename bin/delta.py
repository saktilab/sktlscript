...
fout = open('list_distance.dat', 'w')

step = 0
format_str = ' {:>15s}'*len(final_pairs)
headers = ['{}({})-{}({})'.format(symbols[i], i, symbols[j], j) for i, j in final_pairs ]
print(('{:>10s} ' + format_str + ' {:15s}').format('Time[ps]', *headers, 'delta'), file=fout)
with open(traj_file, 'r') as f:
    for line in f:
        time = dt*step
        step+=1
        ...
        print(('{:>10.4f} ' + format_str + ' {:15.4f}').format(time, *distances, delta),file=fout)
