...
parser = argparse.ArgumentParser(description="Compute IR Spectra from MD Trajectory")
parser.add_argument('-i', '--file_name', type=str)
parser.add_argument('-dt', '--delta_t',default=1.0, type=float)
parser.add_argument('-o', '--outputf', default="ir.dat", type=str)
parser.add_argument('-m','--mode', type=str)
parser.add_argument('-s', '--sel', nargs='+')
parser.add_argument('-n', '--norm', default=False, type=bool)
parser.add_argument('-k', "--kind", default="Gaussian", type=str)
parser.add_argument('-d', '--standard', default=500, type=float)
...
delta_t = float(opt.delta_t) * 1e-15
outputf = opt.outputf
mode = opt.mode
sel = [int(x) for x in opt.sel]
########################
#delta_t = 0.5 * 1e-15
#Fs = 1/delta_t
#T = 50.0
...
def information(fname):
    ''' fetch the number of atoms and the elements information
...
def main(file_path, file_name):
    ...
    window = choose_window(data, opt.kind)
    parsing = time.clock()
...
#    norm_intensity = intensity / max(intensity)
    if(opt.norm == True):
        norm_intensity = intensity/max(intensity)
        save_results(outputf, wavenumber, norm_intensity)
    else:
    #### save the results
