...
frame_str = mg.Structure.from_file(opt.frame)
mole_str = mg.Molecule.from_file(opt.mole)
...
while current_num_gas < N_GAS:
    ...
    for site in new_mole:
        new_site = mg.PeriodicSite(site.specie, site.coords, frame_str.lattice, coords_are_cartesian=True).to_unit_cell
...
# frame_str.sort()
frame_str.to(fmt='poscar', filename='GAS_{}.POSCAR'.format(N_GAS))