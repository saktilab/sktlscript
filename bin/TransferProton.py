...
parser = argparse.ArgumentParser(description='Melacak Proses Transfer Proton')
parser.add_argument('traject', metavar='traject', type=str)
parser.add_argument('nac', metavar='nac', type=str)
parser.add_argument('-p', '--proton', type=bool, default=True,  help='Apakah Sistem Ini Terprotonasi?')
parser.add_argument('-c', '--charge', type=float, default=-0.60, help='Muatan minimum suatu atom dikatakan terprotonasi')

group = parser.add_mutually_exclusive_group(required=True)
group.add_argument('-e', '--elem', type=str, nargs='+', help='Atom yang akan dianalisis')
group.add_argument('-i', '--index', nargs='+', type=int, help='Indeks dari atom yang akan dianalisis')
group.add_argument('-if', '--index-file', type=str, help='Nama file yang mengandung indeks atom yang hendak dianalisis')
...
time = dt*1
fout=open("charge.dat", "w")
print ('#{:>9s} {:>8s} {:>8s} {:>10s} {:>8s} {:>10s} {:>10s}'.format('Time(ps)', '#Proton', 'MaxIndex', 'MaxCharge', 'MinIndex', 'MinCharge', 'AvgCharge'),file=fout)
print ('{:10.4f} {:>8d} {:>8d} {:>10.4f} {:>8d} {:>10.4f} {:>10.4f}'.format(time/1000.0, num_pros, max_charge_atoms[1][1], max_charge_atoms[1][0], min_charge_atoms[1][1], min_charge_atoms[1][0], avg_charge[1]   ),file=fout)
...
for i in range(2, len(max_charge_atoms)):
    ...
    print ('{:10.4f} {:>8d} {:>8d} {:>10.4f} {:>8d} {:>10.4f} {:>10.4f}'.format(time/1000.0, num_pros, max_charge_atoms[i][1], max_charge_atoms[i][0], min_charge_atoms[i][1], min_charge_atoms[i][0], avg_charge[i]),file=fout)
