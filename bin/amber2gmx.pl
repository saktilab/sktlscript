...
{
  my $command = "cat $crd_filename | /usr/local/Caskroom/miniconda/base/bin/ambpdb -atm -p $prmtop_filename > $pdb_filename";
...
{
  my $command = "/usr/local/Caskroom/miniconda/base/bin/rdparm $prmtop_filename < $rdparm_input_filename > $rdparm_output_filename";
  print "Calling rdparm to extract information from prmtop file.\n$command\n" if($debug_flag);
  my $output = `$command`;
  print "$output\n" if($debug_flag);
}
