...
if (opt.merged):
    ...
    with open(opt.filename, 'r') as f:
        for line in f:
            if opt.property == 'temp' and 'MD Temperature' in line:
                value = float(line.split()[4])
                data.append(value)
...
print ('-'*70)
print('{:<6d} {:<15.8f} {:<15.8f} {:<15.8f} {:<15.8f}'.format( len(real_data), statistics.mean(real_data), max(real_data), min(real_data), stddev))
